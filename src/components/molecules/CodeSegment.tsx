...
export default function CodeSegment() {
  const { FULL_NAME, JOB_TITLE, COMPANY, BIRTH_DAY } = DATA;

  const SHARED_CSS =
    "text-md sm:text-xl md:text-2xl lg:text-3xl font-mono w-fit whitespace-nowrap";

  const START_CONTAINER = "flex gap-2";
  const START_CONTAINER_INDENT = "flex gap-2 md:gap-3 lg:gap-4 ml-4 md:ml-8";

  const CIRCLE_CSS = "rounded-full w-3 h-3 md:w-4 md:h-4";
  ...
  return (
    <div className="px-3">
      <div className="bg-[#212529] p-6 rounded-xl flex flex-col h-fit w-fit mx-auto max-w-full">
